(async () => {
	...
	const p = '//fastly.jsdelivr.net',
		...
		R = {css: 'css', 'sanitized-css': 'css', javascript: 'javascript', json: 'javascript', wikitext: 'mediawiki'},
		d = h
			? {
	...
	const q = {'"': 'quot', "'": 'apos', '<': 'lt', '>': 'gt', '&': 'amp', ' ': 'nbsp'},
		...
		v = ({keyMap: e}) => e.default === e.pcDefault,
		X = (e) => {
			var i = v(e) ? 'Ctrl' : 'Cmd';
			...
			if (('mediawiki' === e || 'widget' === e) && x.has('contextmenu')) {
				...
				t.contextmenu(({pageX: e, pageY: i}) => {
					...
					if (/\bmw-(?:template-name|parserfunction)\b/u.test(n)) {
						var s = g.getLineTokens(a);
						for (let e = s.length - 1; 0 < e; e--) {
							var {type: r, end: d, string: l} = s[e];
							s[e - 1].type === r && ((s[e - 1].end = d), (s[e - 1].string += l), s.splice(e, 1));
						}
						var c = s.findIndex(({start: e, end: i}) => e < o && i >= o),
							m = s[c].string
								.replace(/\u200E/gu, '')
								.replace(/_/gu, ' ')
								.trim();
	...
	const te = (e, i = !1) => {
		...
		ae = (e, i) => (Array.isArray(e) ? e.some((e) => i.has(e)) : i.has(e)),
		oe = (e) => {
			let i = [];
			var t,
				a = 'function' == typeof window.CodeMirror,
				o = a ? window.CodeMirror : {modes: {}, prototype: {}, commands: {}, optionHandlers: {}, helpers: {}};
			if (
				...
				for (const n of ['css', 'javascript', 'mediawiki', 'htmlmixed', 'xml'])
					o.modes[n] || (i = i.concat(d[n]));
			else i = i.concat(d[e]);
			return (
		...
		E = (e) => e.flatMap(({aliases: e, name: i}) => e.map((e) => ({alias: e, name: i}))),
		_ = (e) => Object.fromEntries(e.map(({alias: e, name: i}) => [e.replace(/:$/u, ''), i])),
		se = (e) => {
		...
		re = async (e, i) => {
			if ('mediawiki' === e || 'widget' === e) {
				...
				var t = e && Object.values(e.functionSynonyms[0]).includes(!0);
				if (!(e && e.redirect && e.img && e.variants) || t) {
					...
					if (e && !t) {
						var d,
							l,
							[c] = e['functionSynonyms'];
						if (!c.subst)
							for ({alias: d, name: l} of E(o.filter(({name: e}) => m.has(e))))
								c[d.replace(/:$/u, '')] = l;
					} else {
						e = {
		...
		de = {
			...
			setSelection(e) {
				return (
					O.setSelection(O.posFromIndex(e.start), 'end' in e ? O.posFromIndex(e.end) : void 0),
					O.focus(),
					this
				);
			},
			...
			getCaretPosition(e) {
				var i = O.indexFromPos(O.getCursor('from')),
		...
		me =
			(new MutationObserver((e) => {
				var i = $(e.flatMap(({addedNodes: e}) => [...e])).find('#Wikiplus-Quickedit, #Wikiplus-Setting-Input');
	...
	const L = async (e) => {
		if (x.has('otherEditors')) {
			var i = 'text/mediawiki' === (i = e.getOption('mode')) ? 'mediawiki' : i,
				t = te(CodeMirror, !0),
				a = e.getOption('json'),
				{
					prototype: o,
