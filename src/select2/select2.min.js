((A) => {
	...
	const Y = (() => {
			...
			function L(g) {
				...
				if (!C.call(w, g) && !C.call(M, g)) throw Error('No ' + g);
				return w[g];
			}
			function O(g) {
				let h;
			...
			if (A && A.fn && A.fn.select2 && A.fn.select2.amd) var q = A.fn.select2.amd;
			let Q;
			let R;
			let W;
			var E;
			...
			return (
				(q && q.requirejs) ||
					...
					(N = (g, h, e, c) => {
						...
						if ('undefined' == f || 'function' == f) {
							...
							for (a = 0; a < h.length; a += 1)
								if ('require' === (f = (b = S(h[a], k)).f)) d[a] = I.require(g);
								else if ('exports' === f) {
									d[a] = I.exports(g);
									var m = !0;
								} else if ('module' === f) var n = (d[a] = I.module(g));
								else if (C.call(w, f) || C.call(F, f) || C.call(M, f)) d[a] = L(f);
								else {
									if (!b.p) throw Error(g + ' missing ' + f);
					...
					(Q =
						...
						H =
							(g, h, e, c, b) => {
								if ('string' == typeof g) return I[g] ? I[g](h) : L(S(g, h ? O(h) : []).f);
								if (!g.splice) {
									if (((B = g).deps && H(B.deps, B.callback), !h)) return;
					...
					return (
						...
						(e.prototype.displayMessage = function (c) {
							...
							const a = g('<li role="alert" aria-live="assertive" class="select2-results__option"></li>'),
								d = this.options.get('translations').get(c.message);
							a.append(b(d(c.args)));
						...
						(e.prototype.setClasses = function () {
							...
							this.data.current((b) => {
								...
								c.$results.find('.select2-results__option[aria-selected]').each(function () {
									const d = g(this),
										f = h.GetData(this, 'data'),
										k = '' + f.id;
									(null != f.element && f.element.selected) ||
									(null == f.element && -1 < g.inArray(k, a))
						...
						(e.prototype.bind = function (c, b) {
							...
							c.on('results:previous', () => {
								let d = a.getHighlightedResults(),
									f = a.$results.find('[aria-selected]'),
									k = f.index(d);
								if (!(0 >= k)) {
									--k;
									0 === d.length && (k = 0);
							...
							g.fn.mousewheel &&
								this.$results.on('mousewheel', (d) => {
									let f = a.$results.scrollTop(),
										k = a.$results.get(0).scrollHeight - f + d.deltaY;
				...
				q.define('select2/selection/placeholder', ['../utils'], (g) => {
					...
					return (
						(h.prototype.normalizePlaceholder = (e, c) => (
							'string' == typeof c && (c = {id: '', text: c}), c
						)),
						...
						(h.prototype.update = function (e, c) {
							...
							if (1 < c.length || b) return e.call(this, c);
							this.clear();
					...
					return (
						...
						(c.prototype._handleClear = function (b, a) {
							if (
								...
								let d = {data: a};
								if ((this.trigger('clear', d), d.prevented)) this.$element.val(b);
								else {
					...
					return (
						...
						(c.prototype.bind = function (b, a, d) {
							...
							this.$selection.on('keyup.search input.search', '.select2-search--inline', (n) => {
								if (m && 'input' === n.type) f.$selection.off('input.search input.searchcheck');
								else {
									const p = n.which;
				...
				q.define('select2/data/select', ['./base', '../utils', 'jquery'], (g, h, e) => {
					...
					return (
						...
						(c.prototype.item = function (b) {
							let a;
							if (null != (a = h.GetData(b[0], 'data'))) return a;
							if (b.is('option'))
								a = {
									id: b.val(),
									text: b.text(),
					...
					return (
						...
						(c.prototype._applyDefaults = (b) =>
							e.extend(
								...
								{
									data: function (a) {
										return e.extend({}, a, {q: a.term});
									},
									transport: function (a, d, f) {
										a = e.ajax(a);
							...
							null != c.term && null == c.page
								? e.call(this, c, function m(f, k) {
										for (var n = f.results, p = 0; p < n.length; p++) {
											const t = n[p],
												y = null != t.children && !m({results: t.children}, !0);
				...
				q.define('select2/data/tokenizer', ['jquery'], (g) => {
					...
					return (
						(h.prototype.bind = function (e, c, b) {
							e.call(this, c, b);
							this.$search =
								c.dropdown.$search || c.selection.$search || b.find('.select2-search__field');
						}),
						(h.prototype.query = function (e, c, b) {
							const a = this;
							c.term = c.term || '';
				...
				q.define('select2/dropdown/hidePlaceholder', [], () => {
					...
					return (
						...
						(g.prototype.normalizePlaceholder = (h, e) => (
				...
				q.define('select2/i18n/en', [], () => ({
					...
					inputTooLong: function (g) {
						g = g.input.length - g.maximum;
						let h = 'Please delete ' + g + ' character';
						return 1 != g && (h += 's'), h;
					},

					inputTooShort: function (g) {
						return 'Please enter ' + (g.minimum - g.input.length) + ' or more characters';
					},
					...
					(
						...
						return (
							(G.prototype.apply = function (l) {
								...
								if (null == l.selectionAdapter) {
									if (
										...
										null != l.placeholder &&
											(l.selectionAdapter = m.Decorate(l.selectionAdapter, a)),
								...
								return (
									(l.language = u),
									(l.translations = this._processTranslations(l.language, l.debug)),
									l
								);
							}),
							...
							(G.prototype._resolveLanguage = (l) => {
								if (!l || g.isEmptyObject(l)) return [];
								if (g.isPlainObject(l)) return [l];
								l = g.isArray(l) ? l : [l];
				...
				q.define('select2/core', ['jquery', './options', './utils', './keys'], (g, h, e, c) => {
					...
					return (
						...
						(b.prototype._resolveWidth = function (a, d) {
							...
							if ('resolve' == d)
								return (
									(f = this._resolveWidth(a, 'style')),
									null != f ? f : this._resolveWidth(a, 'element')
								);
							if ('element' == d) return (f = a.outerWidth(!1)), 0 >= f ? 'auto' : f + 'px';
							if ('style' != d) return 'computedstyle' != d ? d : window.getComputedStyle(a[0]).width;
							a = a.attr('style');
							...
							for (const k = a.length; d < k; d += 1) {
								...
								if (null !== m && 1 <= m.length) return m[1];
							}
							return null;
						}),
						...
						(b.prototype.trigger = function (a, d) {
							...
							if ((void 0 === d && (d = {}), a in k)) {
								...
								if ((f.call(this, k[a], m), m.prevented)) return void (d.prevented = !0);
							}
							f.call(this, a, d);
						}),
					...
					(g, h, e, c, b) => {
						if (null == g.fn.select2) {
							...
							g.fn.select2 = function (d) {
								...
								if ('string' != typeof d) throw Error('Invalid arguments for Select2: ' + d);
								let f;
