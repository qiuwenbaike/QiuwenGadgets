...
(function logFilter(): void {
	...
	const filterLists = {
		...
		buildForm(): void {
			...
			fieldsetHtml += `<button type="button" value="0">${mw.html.escape(
				mw.msg('filter-filter-button')
			)}</button>`;
			fieldsetHtml += `<button type="button" value="1">${mw.html.escape(
				mw.msg('filter-highlight-button')
			)}</button>`;
