	...
	$(() => {
		...
		class Title extends Stringwrapper {
			...
			fromURL(h) {
				...
				if (email) {
					this.setUtf(
						Title.decodeNasties(
							`${mw.config.get('wgFormattedNamespaces')[pg.nsUserId]}:${new Title(
								email[3]
							).stripNamespace()}`
						)
					);
			...
			namespaceId() {
				...
				const namespaceId =
					mw.config.get('wgNamespaceIds')[
						this.value.slice(0, Math.max(0, n)).split(' ').join('_').toLowerCase()
					];
		...
		class Previewmaker {
			...
			killImages() {
				...
				this.kill(
					new RegExp(`[[][[]\\s*(${forbiddenNamespaceAliases.join('|')})\\s*:`, 'i'),
					/]]\s*/,
					'[',
					']'
				);
		...
		const formattedDateTime = (date) => {
			...
			if (getMWDateFormat() === 'ISO 8601') {
				...
				return `${map(zeroFill, [d2.getFullYear(), d2.getMonth() + 1, d2.getDate()]).join('-')}T${map(
					zeroFill,
		...
		const setTitleBase = () => {
			...
			pg.re.basenames = new RegExp(
				`^(${map(literalizeRegex, [pg.wiki.titlebase, pg.wiki.articlebase]).join('|')})`
			);
		};
		// Global regexps
		const setMainRegex = () => {
		...
		const specialLink = (l) => {
			...
			const base = `${pg.wiki.titlebase + mw.config.get('wgFormattedNamespaces')[pg.nsSpecialId]}:${
				l.specialpage
			}`;
			...
			hint = hint
				? simplePrintf(hint, [safeDecodeURI(l.article)])
				: safeDecodeURI(`${l.specialpage}:${l.article}`);
		...
		const getHistory = (wikipage, onComplete) => {
			...
			const url = `${
				pg.wiki.apiwikibase
			}?format=json&formatversion=2&action=query&prop=revisions&titles=${new Title(
				wikipage
		...
		pg.string = {
			...
			defaultpopupRevertSummary:
				'Revert to revision %s using [[:enwiki:Wikipedia:Tools/Navigation_popups|popups]]',
