...
(function twinklecopyvio() {
	const {wgArticleId, wgCurRevisionId, wgDiffNewId, wgNamespaceNumber, wgPageName, wgRevisionId} = mw.config.get();
	/**
	...
	Twinkle.copyvio.callbacks = {
		...
		main: (pageobj) => {
			...
			if (params.notify) {
				...
				const notifytext = '\n{{'.concat('subst:', `CopyvioNotice|${wgPageName}}}`);
		...
		taggingArticle: (pageobj) => {
			...
			if (params.g4) {
				...
				if (params.g4_pagename) {
					...
					speedyTag += `|c1=[[Special:Undelete/${wgPageName}]]`;
				}
				speedyTag += '}}';
				tag = `${speedyTag}\n${tag}`;
			}
		...
		copyvioList: (pageobj) => {
			...
			output += '\n{{'.concat('subst:', `CopyvioVFDRecord|${wgPageName}}}`);
			pageobj.setAppendText(output);
			pageobj.setEditSummary(`加入[[${wgPageName}]]`);
			pageobj.setChangeTags(Twinkle.changeTags);
			pageobj.setCreateOption('recreate');
			pageobj.append();
		},
