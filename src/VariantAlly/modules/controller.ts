...
function rewriteLink(link: string, variant: Variant): string {
	try {
		...
		if (REGEX_WIKI_URL.test(path)) {
			url.path = `/${variant}/${url.path.replace(REGEX_WIKI_URL, '')}`;
			delete query['variant']; // For things like /zh-cn/A?variant=zh-hk
		} else {
...
function checkThisPage(preferredVariant: Variant, pageVariant?: Variant): void {
	...
	redirectionURL.path = `/wiki/${redirectionOrigin}`;
...
function applyURLVariant(): void {
	const variant = new mw.Uri().query[VARIANT_PARAM]?.toString();
